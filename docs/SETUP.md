...
## 二之二、另一條路：整套跑在投影幕那台電腦上

預設是「網站在 GitHub Pages、伺服器在 Fly.io 東京、玩家走自己的行動網路」。
如果當天想改成**全部跑在現場那台筆電上**，可以，但要整套一起搬：

只把伺服器搬到筆電、網站留在 GitHub Pages 是**行不通的** —— Pages 是 HTTPS，
瀏覽器會擋掉 HTTPS 頁面連 `ws://` 的區網位址，而區網 IP 拿不到憑證所以也
不能用 `wss://`。網站必須跟伺服器從同一台機器用 http 發出去，同源才沒有這個問題。

```bash
npm install && VITE_WS_URL=auto npm run build && npm run host
```

`VITE_WS_URL=auto` 的意思是「連回這一頁的同一個來源」。因為現場的網址是筆電的
區網 IP，build 的當下不會知道，寫死的話每換一個場地就要重 build。

跑起來會印出三個網址（投影幕／玩家入口／主控台），把**玩家入口**做成 QR 貼出去。
用 `localhost` 開投影幕也沒關係，它會自動把你導到區網 IP，QR 才不會編出
只有那台電腦連得到的網址。

| | 好處 | 代價 |
|---|---|---|
| Fly.io（預設） | 玩家各走各的行動網路，不靠場地 | 延遲約 60ms |
| 現場自架 | 延遲 1–5ms，對外斷網也能玩 | **全場必須連同一個 Wi-Fi** |

⚠️ **代價那一欄才是重點。** 當初選行動網路就是為了避開「一台 AP 撐 100 支手機」
這件事，消費級機種常常撐不住。要走這條路，開場前一定要在現場實測：
100 支手機同時連上那個 AP，而不是相信規格表。

主控台的 Google 登入在這個模式下要另外設環境變數（`GOOGLE_CLIENT_ID`、
`ALLOWED_EMAILS`），不設的話伺服器會在啟動時警告「任何人都能控制投影幕」。
區網內、活動當下，這個風險通常可以接受，但要知道它是關著的。
...
| 項目 | 為什麼 |
|---|---|
| **備用筆電**，已開好頁面待命 | 主機掛掉時自動接手。兩台可以同時開著，備用機每 3 秒去問一次，主機一斷就接手（右下角會寫「已接手」），分數與地理題庫也會一起接回去 |
...
| 跳出來的字 | 意思 | 怎麼辦 |
|---|---|---|
| ⚠️ 沒送出去 —— 連線中斷 | 主控台自己斷線了 | 等幾秒會自動重連，畫面上方的紅色橫條消失就好了 |
| ⚠️ 投影幕不在線上 | 伺服器收到了，但沒有任何投影幕連著 | 去看投影幕那台，重新整理 `stage.html` |
| ⚠️ 投影幕沒有回應 | 指令送到了，但兩秒內投影幕的狀態沒跟上 | 多半是投影幕那台卡住，重新整理 |
| 這一關不能暫停 | 正常，不是故障 | 限時作答的關卡沒有暫停，要提早結束按「下一題／下一關」 |

「開始」按鈕會顯示投影幕回報的真實狀態（**▶ 開始** ／ **▶ 進行中**），
那是投影幕說的，不是「你按過了」。所以**按幾次都沒關係** ——
開始和暫停是分開的兩顆，重複按不會把剛開始的一局關掉。

**有些玩家動得了、有些不動**
玩家的手機在換基地台或 Wi-Fi 切行動網路之後，連線會變成「看起來還連著、
其實已經死了」。程式每 3 秒會自己確認一次，10 秒沒回音就重連，所以
最多卡 10 秒會自己好。請那個人**不要重新整理**（重整要重新選隊），
看畫面上方的字變回「已連線」就好。

**兩台投影幕都開著，主機掛了**
備用機每 3 秒會自己去搶主控權，接手之後右下角會寫「已接手，這台是主投影幕」。
不用手動做什麼，但要記得把投影機切到那一台。
...
**投影幕重開了，分數與題目會不會不見**
不會。伺服器留著最後一張計分表、以及主控台編好的地理題庫與照片，
接手的那台一連上就會全部接回去。兩個都實測過。

房間在沒人連線之後還會留 **10 分鐘**才真的清掉。這一條很重要：
投影幕重整的那一兩秒，如果剛好主控台也關著、手機還沒進來，
房間被立刻刪掉的話接手機制就等於沒有 —— 而那正是它要處理的情境本身。

伺服器自己重啟的話（`fly deploy`、機器重開）它的記憶會清空，
但主控台的題庫另外存在它自己的瀏覽器裡，一連上就會自動推回去。
所以真的全部重來時，開一次主控台就好。
