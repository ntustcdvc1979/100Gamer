...
實測（120 個客戶端 × 20 Hz，本機）：**p95 = 31ms，零連線失敗**。
...
```bash
npm run loadtest:ws -- --url wss://ntustcdvc-100gamer.fly.dev --clients 120 --seconds 600
```
...
| 看什麼 | 過關標準 |
|---|---|
| 延遲 p95 | **< 300ms**。本機基準是 31ms，打到東京大約 +40ms |
| 連線失敗數 | 0 |
| 投影幕幀率 | 畫 120 個物件不能掉幀 |
| `fly logs` / `fly status` | 記憶體沒有一路往上（漏連線） |

如果 p95 太高，第一個該調的是 `server/index.js` 的 `FLUSH_HZ`——
攤平的間隔會**整個加到延遲上**。20 Hz 時本機實測 p50 就有 64ms，改成 60 Hz 降到 30ms。
