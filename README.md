...
| `play.html` | 玩家手機。輸入名字、自動分隊、搖桿／拍照／翻面。 |
| `console.html` | **主控台**。主持人用手機或平板遙控投影幕、看全場分數、叫排行榜。限定指定的 Google 帳號。 |
...
| 4 | **拍照找顏色** | 個人賽 | 出題「蘋果紅」，60 秒內找一個最像的東西拍下來。可以一直重拍，但**只能上傳一次**。分數時間到才公布，投影幕依高低秀出照片。三題 |
| 5 | **火候達人** | 個人賽 | 「煎素火腿，10 秒翻面」。投影幕上的秒數**兩秒後就淡掉**，要自己數。第二題是炸薯條起鍋，比的是把手機提起來的時間。三題 |

分數跨關卡累計，主控台隨時看得到，按一下就投在投影幕上。
...
- **拍照只送縮圖，不送原圖。** 要在投影幕上秀照片，照片就一定得離開手機。
  代價壓在 200px、JPEG 0.55（約 8–12KB）：100 張約 1MB，一次性，
  256MB 的中繼機吃得下；換成原圖是幾十 MB，會直接打爆。見下面的「資料與隱私」。
- **分數等時間到才公布。** 邊拍邊看分數的話，大家會站在原地對著同一個東西
  微調角度刷分，而不是跑去找更像的東西。
- **色差用 Lab 空間的 CIE94，不是 RGB 歐氏距離。** RGB 距離跟人眼差很遠 ——
  深藍和黑在 RGB 上很近，看起來卻完全不同。
- **火候達人的秒數會淡掉。** 這一關的樂趣就是用身體去數秒，把碼表放在畫面上就沒了。
  數字、進度圈、連鍋子的熱度都在兩秒內收掉，不留任何計時線索。
- **火候達人一定有按鈕備援。** iOS 的感測器權限被拒絕之後要重新載入頁面才能再問，
  100 人裡卡住 10% 就是現場 10 個人舉手。action 裡會記錄是 motion 還是 tap。
- **手機上沒事不顯示搖桿。** 只有真的要用搖桿的關卡才長出來，
  其他時候是一句「看投影幕，等主持人」。憑空出現的搖桿只會讓人亂推。
...
src/
  ...
  stage/
    ...
    games/          五個遊戲，各自實作 Game 介面
  play/
    main.ts         加入、分隊，依 state.control 換操作介面
    input.ts        虛擬搖桿
    camera.ts       拍照取色（照片不上傳）
    flip.ts         翻面偵測（加速度計 z 軸 + 按鈕備援）
  console/          主控台：Google 登入、遙控、計分表
  config/           隊數、頻率、題庫
  shared/           隊伍、節流、QR、色差、共用樣式
server/
