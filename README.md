...
| 4 | **拍照找顏色** | 個人賽 | 出題「蘋果紅」，60 秒內在現場找一個最像的東西拍下來，比色差給分。三題 |
| 5 | **火候達人** | 個人賽 | 「煎素火腿，10 秒翻面」。玩家在指定秒數把手機翻面，誤差越小分數越高。三題 |
...
- **拍照找顏色的照片不上傳。** 顏色在手機上算完（[`src/play/camera.ts`](src/play/camera.ts)），
  只送一個 hex 上來，幾十個 bytes。100 支手機傳照片會壓垮一台 256MB 的中繼伺服器，
  而且拍到別人的臉就變成個資問題。
